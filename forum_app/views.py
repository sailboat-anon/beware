...
def index(request):
    ...
    for thread in threads:
        ...
        thread.abridged_replies = list(thread.replies.all())[-5:]
        if num > 5:
            thread.hidden = num - 5
...
def reply(request, post_id):
    ...
    if type(post) is list:
        for error in post:
            messages.add_message(request, messages.ERROR, error)
    return redirect("/b/thread/{}".format(post_id))
...
def thread(request, thread_id):
    try:
        ...
        full_path = "localhost:8000/b/thread/{}".format(thread_id)
...
def query(request, post_id):
    ...
    if len(post) == 0:
        ...
        url = "/b/thread/{}#{}".format(post_id, post_id)
    else:
        url = "/b/thread/{}#{}".format(post[0].replies.all()[0].id, post_id)
